...
defmodule VenueScraper do
  ...
  def get_bands_for(user \\ "SP420") do
    url =
      @lastfm_url <>
      "?method=user.gettopartists&user=#{user}&api_key=#{@lastfm_api_key}&format=json&period=overall&limit=500"
  ...
  def catil do
    ...
    |> Enum.map(
         fn li ->
           ...
           presents =
             Floki.find(li, "div:nth-child(2) p.event-results-sponsor")
             |> Floki.text()

           title =
             Floki.find(li, "div:nth-child(2) h3")
             |> Floki.text()
           ...
           description =
             Floki.find(li, "div:nth-child(2) p.description")
             |> Floki.text()
           ...
           starts_at_raw =
             Floki.find(li, "div.event-date span")
           ...
           starts_at =
             Timex.parse(
               "#{starts_at_raw} #{Timex.Timezone.Local.lookup()}",
               "{YYYY}-{0M}-{0D}T{h24}:{m}:{s} {Zname}"
             )
             |> elem(1)
  ...
  def mohawk do
    ...
    |> Enum.map(
         fn div ->
           ...
           doors_at =
             Timex.parse(
               "#{raw_date} #{raw_time} #{Timex.Timezone.Local.lookup()} #{Timex.today().year}",
               "%a %b %e %l:%M%p %Z %Y",
               :strftime
             )
             |> elem(1)
           ...
           presents =
             Floki.find(billing_div, "h6")
             |> List.first()
             |> (fn
                   presents when is_nil(presents) ->
                     nil

                   presents when is_tuple(presents) ->
                     elem(presents, 2)
                     |> List.first()
                 end).()

           title =
             Floki.find(billing_div, "h1")
             |> List.first()
             |> elem(2)
             |> List.first()
           ...
           bands =
             Floki.find(billing_div, "h5")
             |> List.first()
             |> (fn
                   bands when is_nil(bands) ->
                     nil

                   bands when is_tuple(bands) ->
                     elem(bands, 2)
                     |> Enum.filter(fn band -> is_bitstring(band) && band != "More" end)
                 end).()
  ...
  # Barracuda: https://www.eventbrite.com/venue/api/feeds/venue/640.json
  # Sidewinder: https://www.eventbrite.com/venue/api/feeds/venue/252.json (Going out of business???? never anything)
  def queueapp_json_for(
        url \\ "https://www.eventbrite.com/venue/api/feeds/venue/640.json",
        venue_name \\ "Barracuda"
      ) do
    keys = ~W[title presents description id starts_at doors_at performing]

    File.read!("./lib/20180722_barracuda.json")
    #HTTPoison.get!(url).body
    |> Poison.decode!()
    |> Enum.map(
         fn json ->
           ...
           doors_at =
             Timex.parse(
               "#{Map.get(json, "starts_at")} #{Map.get(json, "doors_at")} #{Timex.Timezone.Local.lookup()}",
               "{YYYY}-{0M}-{0D} {h24}:{m} {Zname}"
             )
             |> elem(1)
             |> Timex.format("{ISO:Extended:Z}")
             |> elem(1)
  ...
  # TODO: Replace below with elastix
